...
export async function getScoutProfile(userId: string): Promise<ScoutProfile> {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id, name, education_level, field_of_study, skills, interests, location, remote_ok, opportunity_types, experience_level",
    )
    .eq("id", userId)
    .maybeSingle();
  ...
  if (!data)
    throw new Error("Complete your profile before scouting opportunities.");
...
export async function getPreFetchedFallback(
  ...
  const { data, error } = await supabase
    .from("opportunities")
    .select(
      "id, title, organization, summary, source_url, type, deadline, compensation, required_skills, education_level, experience, location, remote_ok, other_criteria",
    )
...
export async function persistLiveCandidate(
    ...
    .upsert(
      {
        ...
        summary:
          ...
          "No description was available from the source.",
        ...
        remote_ok:
          candidate.isRemote ?? candidate.eligibility.remoteOk ?? false,
  ...
  if (error) {
    console.error("[supabase] persist opportunity failed:", {
    ...
    throw new Error("Unable to persist scouted opportunities.");
  }
  if (!data) {
