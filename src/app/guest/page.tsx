...
export default function GuestPage() {
  ...
  return <main className="grain min-h-dvh bg-background px-5 py-8 md:px-8 md:py-12">
    <div className="mx-auto max-w-[1040px]">
      <header className="flex items-center justify-between"><Link href="/login" className="focus-ring flex items-center gap-2.5"><Image src="/icon.jpg" alt="" width={40} height={40} className="size-10" priority /><span className="text-lg font-bold tracking-[-.06em]">Scout<span className="text-[#486257]">Deck</span></span></Link><Link href="/login" className="focus-ring text-xs font-semibold text-primary underline underline-offset-4">Sign in</Link></header>
      <section className="atlas-frame mt-8 overflow-hidden px-6 py-8 text-sidebar-foreground md:px-10 md:py-10"><div className="atlas-route -right-20 -top-36 size-[390px]" /><div className="relative max-w-[650px]"><p className="signal-label text-primary">Guest field / private to this browser</p><h1 className="mt-4 text-balance text-[clamp(2.7rem,5vw,4.6rem)] font-medium leading-[.92] tracking-[-.06em]">Find a few good <span className="signal-display text-primary">directions</span>.</h1><p className="mt-4 max-w-[510px] text-sm leading-relaxed text-sidebar-foreground/70">Try ScoutDeck without an account. Your profile and results stay in this browser until you choose to create one.</p></div></section>
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <form onSubmit={submit} className="rounded-[24px] border border-card-border bg-card p-5 shadow-[0_7px_0_hsl(var(--foreground)/.04)] md:p-7">
          ...
          <div className="mt-5"><p className="text-xs font-semibold">What should we scout?</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{opportunityTypes.map((type) => { const active = profile.opportunityTypes.includes(type.value); return <button key={type.value} type="button" aria-pressed={active} onClick={() => toggleType(type.value)} className={`focus-ring flex items-center justify-between rounded-xl border px-3 py-2.5 text-left text-xs font-medium ${active ? 'border-primary bg-primary/10 text-primary' : 'border-border hover:bg-muted'}`}>{type.label}{active && <Check size={14} />}</button>; })}</div></div>
        ...
        <section aria-live="polite"><div className="flex items-end justify-between border-b border-border pb-4"><div><p className="signal-label text-[#486257]">Guest shortlist</p><h2 className="mt-2 text-2xl font-medium tracking-[-.055em]">{context}</h2></div><span className="font-mono-label text-[10px] uppercase tracking-[.12em] text-muted-foreground">Local only</span></div>{matches.length ? <div className="mt-5 space-y-4">{matches.map((match) => <article key={match.candidateId} className="border-y border-card-border bg-card p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-muted-foreground">{match.opportunity.organization ?? 'Opportunity'}</p><h3 className="mt-1 text-lg font-semibold tracking-[-.03em]">{match.opportunity.title}</h3></div><span className="shrink-0 font-mono-label text-xs text-primary">{Math.round(match.score)} match</span></div><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{match.matchReason}</p><a href={match.opportunity.sourceUrl} target="_blank" rel="noreferrer" className="focus-ring mt-4 inline-flex items-center gap-1 text-xs font-semibold text-primary underline underline-offset-4">View source <ArrowUpRight size={13} /></a></article>)}</div> : <div className="mt-5 border-y border-dashed border-border px-5 py-12 text-center"><Sparkles size={20} className="mx-auto text-primary" /><h3 className="mt-4 font-semibold">Your private shortlist will appear here.</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">Fill in a few details, then we’ll show the strongest currently available matches.</p></div>}</section>
      </div>
    </div>
  </main>;
}
