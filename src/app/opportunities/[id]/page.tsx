...
export default function OpportunityDetail() {
  ...
  if (query.isError || !opportunity) return <div className="py-14 text-center sm:py-20"><p className="font-mono-label text-[10px] uppercase tracking-[.18em] text-destructive">Signal unavailable</p><h1 className="mt-3 text-2xl font-semibold">This trail has gone cold.</h1><Link href="/dashboard" className="focus-ring mt-6 inline-flex min-h-11 items-center rounded-full bg-primary px-4 text-sm font-semibold text-primary-foreground" data-testid="link-back-discover">Back to discover</Link></div>;
  return (
    <article>
      <button type="button" onClick={() => router.push('/dashboard')} className="focus-ring mb-6 inline-flex min-h-11 items-center gap-2 rounded-full px-1 text-xs font-semibold text-muted-foreground hover:text-foreground sm:mb-8" data-testid="button-back-opportunities"><ArrowLeft size={15} /> Back to shortlist</button>
      <div className="grid gap-8 lg:grid-cols-[1fr_340px] lg:items-start">
        <div>
          ...
          <h1 className="mt-5 max-w-[760px] wrap-anywhere text-balance text-[clamp(2.45rem,10vw,4.6rem)] font-semibold leading-[.98] tracking-[-.06em] md:tracking-[-.065em]">{opportunity.title}</h1>
          ...
          ].map(({ icon: Icon, label, value }) => <div key={label} className="flex min-w-0 gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-muted text-primary"><Icon size={16} /></span><div className="min-w-0"><p className="font-mono-label text-[10px] uppercase tracking-[.12em] text-muted-foreground">{label}</p><p className="mt-1 wrap-anywhere text-sm font-medium">{value}</p></div></div>)}</div></section>
