...
const fitFilters = [
  ...
  { value: 'best', label: 'Best fit · 80+' },
  { value: 'good', label: 'Good fit · 60+' },
] as const;
...
export default function DashboardPage() {
  ...
  const [fitFilter, setFitFilter] = useState<(typeof fitFilters)[number]['value']>('all');
  ...
  const filtered = useMemo(() => (visible ?? []).filter((item) => {
    ...
    const matchesFit = fitFilter === 'all' || (fitFilter === 'best' ? item.score >= 80 : item.score >= 60 && item.score < 80);
    return matchesFilter && matchesFit && haystack.includes(query.toLowerCase());
  }), [filter, fitFilter, query, visible]);
  ...
  return <div>
    <section className="atlas-frame overflow-hidden px-5 py-7 text-sidebar-foreground sm:px-6 sm:py-8 md:px-10 md:py-10"><div className="atlas-route -right-20 -top-36 size-[390px]" /><div className="relative max-w-[720px]"><p className="signal-label text-primary">Live field / your scout report</p><h1 className="mt-5 text-balance text-[clamp(2.4rem,9.5vw,4.8rem)] font-medium leading-[.9] tracking-[-.055em] sm:tracking-[-.06em]">A few good <span className="signal-display text-primary">directions</span>, {firstName}.</h1><p className="mt-5 max-w-[500px] text-sm leading-relaxed text-sidebar-foreground/70 md:text-base">We sorted the noise against what you told us. Start with the one that makes you a little curious.</p></div><div className="relative mt-8 flex items-center gap-3 border-t border-sidebar-border pt-5 text-xs text-sidebar-foreground/70"><Sparkles size={15} className="shrink-0 text-primary" /><span>{opportunityCount ? `${opportunityCount} signals in your orbit` : 'Your first signals are on their way'}</span></div></section>
    <section className="mt-8 sm:mt-12"><div className="flex flex-col justify-between gap-5 border-b border-border pb-6 md:flex-row md:items-end"><div><p className="signal-label text-[#486257]">The short list</p><h2 className="mt-3 text-3xl font-medium tracking-[-.055em]">Worth your next hour</h2></div><div className="relative w-full md:w-[270px]"><Search size={16} className="pointer-events-none absolute left-3.5 top-3.5 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search your shortlist" className="focus-ring h-11 w-full border-b border-foreground bg-transparent pl-10 pr-1 text-sm outline-none placeholder:text-muted-foreground" /></div></div><div className="mt-5 flex flex-wrap items-center gap-3 border-y border-border py-3"><button type="button" onClick={() => scout.scout()} disabled={scout.pending || profileQuery.isLoading} className="focus-ring inline-flex h-11 items-center gap-3 bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-60"><Sparkles size={16} /> {scout.pending ? 'Scouting…' : 'Run a fresh scout'}</button><p className={`min-w-0 text-xs ${scout.error ? 'text-destructive' : 'text-muted-foreground'}`}>{scout.error ?? scout.progress?.message ?? 'Searches and ranks opportunities using your saved profile.'}</p></div><div className="mt-6 flex flex-wrap items-center gap-2 border-b border-border pb-4" role="group" aria-label="Filter opportunities"><Filter size={15} className="mr-1 shrink-0 text-muted-foreground" />{filters.map((item) => <button key={item} type="button" onClick={() => setFilter(item)} className={`focus-ring min-h-11 border-b px-3 py-2 signal-label transition-colors ${filter === item ? 'border-primary text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>{item}</button>)}</div><div className="mt-4 flex flex-wrap items-center gap-2" role="group" aria-label="Filter by match score">{fitFilters.map((item) => <button key={item.value} type="button" aria-pressed={fitFilter === item.value} onClick={() => setFitFilter(item.value)} className={`focus-ring min-h-11 rounded-full border px-3 py-2 text-xs font-semibold transition-colors ${fitFilter === item.value ? 'border-primary bg-primary text-primary-foreground' : 'border-border text-muted-foreground hover:border-primary hover:text-primary'}`}>{item.label}</button>)}</div></section>
    {opportunitiesQuery.isLoading ? <div className="mt-6 grid gap-5 md:grid-cols-2"><SkeletonCard /><SkeletonCard /><SkeletonCard /><SkeletonCard /></div> : opportunitiesQuery.isError ? <div className="mt-6 rounded-[22px] border border-destructive/30 bg-destructive/5 p-8 text-center"><h3 className="font-semibold">The signal went quiet.</h3><p className="mt-2 text-sm text-muted-foreground">We couldn&rsquo;t load your shortlist right now.</p><button type="button" onClick={() => opportunitiesQuery.refetch()} className="focus-ring mt-5 inline-flex items-center gap-2 rounded-full bg-foreground px-4 py-2.5 text-xs font-semibold text-background"><RefreshCw size={14} /> Try again</button></div> : filtered.length === 0 ? <div className="mt-6 rounded-[22px] border border-dashed border-border bg-card/60 px-6 py-14 text-center"><div className="mx-auto grid size-12 place-items-center rounded-2xl bg-accent/40 text-accent-foreground"><Search size={20} /></div><h3 className="mt-5 text-lg font-semibold tracking-[-.03em]">{query || filter !== 'All' || fitFilter !== 'all' ? 'Nothing in this slice yet.' : 'Your scout deck is still warming up.'}</h3><p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground">{query || filter !== 'All' || fitFilter !== 'all' ? 'Try another filter or clear your search.' : 'Complete your profile and we’ll start looking for a strong first match.'}</p>{(query || filter !== 'All' || fitFilter !== 'all') && <button type="button" onClick={() => { setQuery(''); setFilter('All'); setFitFilter('all'); }} className="focus-ring mt-5 text-xs font-semibold text-primary underline underline-offset-4">Clear filters</button>}</div> : <div className="mt-6 grid gap-5 md:grid-cols-2">{filtered.map((opportunity, index) => <OpportunityCard key={opportunity.id} opportunity={opportunity} index={index} />)}</div>}
  </div>;
}
