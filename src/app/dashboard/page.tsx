...
export default function DashboardPage() {
  ...
  const streamed = useMemo<Opportunity[]>(() => scout.matches.map((match) => ({
  ...
  // Prefer the just-streamed shortlist so a fresh scout is visible immediately,
  // even before the database refetch finishes or when persistence is delayed.
  const visible = useMemo(
    () => scout.matches.length > 0 ? streamed : (opportunitiesQuery.data ?? []),
    [opportunitiesQuery.data, scout.matches.length, streamed],
  );
  ...
  const profileReady = Boolean(
    ...
    && (profileQuery.data.skills.length || profileQuery.data.interests.trim())
    && profileQuery.data.opportunityTypes.length,
  );

  if (!profileQuery.isLoading && !profileReady) {
    return <div className="max-w-[850px]"><section className="atlas-frame overflow-hidden px-6 py-8 text-sidebar-foreground md:px-10 md:py-10"><div className="atlas-route -right-20 -top-36 size-[390px]" /><div className="relative max-w-[640px]"><p className="signal-label text-primary">Set your direction</p><h1 className="mt-5 text-balance text-[clamp(2.8rem,5vw,5rem)] font-medium leading-[.9] tracking-[-.06em]">Tell us what you&rsquo;re <span className="signal-display text-primary">moving toward.</span></h1><p className="mt-5 max-w-[520px] text-sm leading-relaxed text-sidebar-foreground/70 md:text-base">A little context lets ScoutDeck find fellowships, builder programs, hackathons, ambassador programs, scholarships, and grants that are actually worth opening.</p></div></section><div className="mt-10"><ProfilePage /></div></div>;
  }
