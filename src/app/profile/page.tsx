...
const opportunityTypes = [
  { value: 'fellowship', label: 'Fellowships' }, { value: 'builder_program', label: 'Builder programs' }, { value: 'ambassador_program', label: 'Ambassador programs' }, { value: 'hackathon', label: 'Hackathons' },
  { value: 'scholarship', label: 'Scholarships' }, { value: 'grant', label: 'Grants' },
];
...
export default function Profile() {
  ...
  const form = useForm<ProfileForm>({ defaultValues: { name: '', educationLevel: '', fieldOfStudy: '', skills: '', interests: '', location: '', remoteOk: true, opportunityTypes: ['fellowship', 'builder_program', 'hackathon'], experienceLevel: 'student' } });
  ...
  const onSubmit = (data: ProfileForm) => {
    ...
    if (!data.fieldOfStudy.trim()) {
      form.setError('fieldOfStudy', { message: 'Add your field so we can make the search relevant.' });
      return;
    }
    if (!skills.length && !data.interests.trim()) {
      const message = 'Add skills or interests so ScoutDeck has a useful search focus.';
      form.setError('skills', { message });
      form.setError('interests', { message });
      return;
    }
    if (!data.opportunityTypes.length) {
      form.setError('opportunityTypes', { message: 'Choose at least one kind of opportunity.' });
      return;
    }
    const payload: ProfileInput = { ...data, skills, opportunityTypes: data.opportunityTypes as ProfileInput['opportunityTypes'], experienceLevel: data.experienceLevel as ProfileInput['experienceLevel'] };
  ...
  return <div className="max-w-[850px]"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="font-mono-label text-[10px] uppercase tracking-[.2em] text-primary">Your starting point</p><h1 className="mt-2 text-[2.65rem] font-semibold leading-none tracking-[-.065em]">Your profile.</h1><p className="mt-4 max-w-[500px] text-sm leading-relaxed text-muted-foreground">Tell ScoutDeck what a good next step looks like. You can change this anytime as your direction sharpens.</p></div><div className="flex items-center gap-3"><div className="relative size-14"><svg viewBox="0 0 36 36" className="size-14 -rotate-90"><circle cx="18" cy="18" r="15.5" fill="none" stroke="hsl(var(--secondary))" strokeWidth="3" /><circle cx="18" cy="18" r="15.5" fill="none" stroke="hsl(var(--primary))" strokeWidth="3" strokeDasharray={`${completion} 100`} strokeLinecap="round" /></svg><span className="absolute inset-0 grid place-items-center font-mono-label text-[10px]">{completion}%</span></div><div><p className="text-xs font-semibold">Signal strength</p><p className="mt-0.5 text-xs text-muted-foreground">{completion >= 80 ? 'Looking clear.' : 'A little more context helps.'}</p></div></div></div>
    <Form {...form}><form onSubmit={form.handleSubmit(onSubmit)} className="mt-9 space-y-6">
      ...
      {saveError && <p className="flex items-center gap-2 text-sm text-destructive" role="alert" data-testid="status-profile-save-error"><CircleHelp size={15} /> {saveError}</p>}
